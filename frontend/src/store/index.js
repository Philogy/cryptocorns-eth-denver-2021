...
import MockLeverERC20 from '@/assets/abis/MockLeverERC20'
...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async loadTokens({ state, getters }) {
      const allTokens = getLeverTokens()

      const currentTokens = allTokens.filter(
        ({ network }) => network === getters.network.name.toLowerCase()
      )
      for (let token of currentTokens) {
        token.contract = new state.web3.eth.Contract(MockLeverERC20.abi, token.address)
        token.balance = new BN(await token.contract.methods.balanceOf(getters.mainAccount).call())
      }

      state.tokenData = currentTokens
    },
